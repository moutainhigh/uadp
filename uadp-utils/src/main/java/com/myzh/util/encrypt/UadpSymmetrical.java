...
import com.myzh.util.base64.UrlBase64;
...
public class UadpSymmetrical implements UadpEncrypt {
	...
	public UadpSymmetrical() {

	}

	public UadpSymmetrical(Algorithm algorithm) {
		this.algorithm = algorithm;
		logger.info("Your current encryption algorithm is " + algorithm.getKey());
	}

	public String encrypt(String value, String key) throws Exception {
		byte[] b = UrlBase64.encode(encrypt(algorithm, value.getBytes(encoding), key));
		return new String(b, encoding);
	}

	public String decrypt(String value, String key) throws Exception {
		byte[] b = decrypt(algorithm, UrlBase64.decode(value.getBytes(encoding)), key);
		return new String(b, encoding);
	}
