...
import com.myzh.util.base64.Base64;
import com.myzh.util.exception.AESException;
...
public class AESMsgCrypt {
	...
	public String encrypt(String text) throws AESException {
		...
		byte[] randomStrBytes = RandomUtils.getCharacterAndNumber(16).getBytes(CHARSET_ENCODING);
	...
	public String decrypt(String text) throws AESException {
		...
		try {
			...
			xmlContent = new String(Arrays.copyOfRange(bytes, 20, 20 + xmlLength), CHARSET_ENCODING);
	...
	protected String encryptMsg(String replyMsg, String timeStamp, String nonce, boolean isXMLParser) throws AESException {
		...
		/* 判断 XML 还是 JSON 解析 */
		if (isXMLParser) {
			return null;//return XMLParser.generate(encrypt, signature, timeStamp, nonce);
		} else {
			return null;//return JSONParser.generate(encrypt, signature, timeStamp, nonce);
		}
	}
