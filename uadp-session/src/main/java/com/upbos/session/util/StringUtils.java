package com.upbos.session.util;
...
public class StringUtils {
     ...
     *                          will not consider subsequent delimiters as session in the first place).
     ...
     * @param retainQuotes   if the quotes themselves should be retained when constructing the corresponding session
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            if (c == beginQuoteChar) {
                ...
                if (inQuotes  // we are in quotes, therefore there can be escaped quotes in here.
                    ...
                    // put one on the session. we do *not* exit the quoted text.
                ...
                tokens.add(s);
                sb = new StringBuilder(); // start work on next session
            } else {
                sb.append(c);
            }
        }
